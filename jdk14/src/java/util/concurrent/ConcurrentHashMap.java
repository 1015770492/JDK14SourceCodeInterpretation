...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    private static final long serialVersionUID = 7249069246763182397L; // 序列化
    private static final int MAXIMUM_CAPACITY = 1 << 30;// 最大容量2的30次方大概10亿
    private static final int DEFAULT_CAPACITY = 16;     // 默认容量16
    static final int MAX_ARRAY_SIZE = Integer.MAX_VALUE - 8;// 最大数组长度
    private static final int DEFAULT_CONCURRENCY_LEVEL = 16;// 默认的并发级别，这个参数的定义是考虑之前的版本兼容，起于jdk6，终止于jdk8
    private static final float LOAD_FACTOR = 0.75f;         // 加载因子
    static final int TREEIFY_THRESHOLD = 8;                 // 树化阈值8
    static final int UNTREEIFY_THRESHOLD = 6;               // 保持树结构的阈值6
    static final int MIN_TREEIFY_CAPACITY = 64;             // 树化的形成条件，最小容量阈值64
    ...
    static final int HASH_BITS = 0x7fffffff; // 留下最高位,目的是为了后面按位与得到符合位
    static final int NCPU = Runtime.getRuntime().availableProcessors(); // 获取cpu核心数
    transient volatile Node<K, V>[] table; // 底层数组
    ...
    private transient KeySetView<K, V> keySet;      // key的set集合
    private transient ValuesView<K, V> values;      // value的集合
    private transient EntrySetView<K, V> entrySet;  // entry集合
    ...
    public ConcurrentHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
        ...
        long size = (long) (1.0 + (long) initialCapacity / loadFactor);// 计算map的容量值,而初始容量就是我们实际要存的元素个数
        // 如果容量超过了最大值则设置为最大值,否则就计算出
    ...
    static final int spread(int h) {
        return (h ^ (h >>> 16)) & HASH_BITS;// HASH_BITS=0x7fffffff 相当于符合位
    }
    ...
    static final class TreeNode<K, V> extends Node<K, V> {
         ...
         */
        final TreeNode<K, V> findTreeNode(int h, Object k, java.lang.Class<?> kc) {
            if (k != null) {
                TreeNode<K, V> p = this;
                do {
                    int ph, dir;
    ...
    static {
        ...
        java.lang.Class<?> ensureLoaded = LockSupport.class;
